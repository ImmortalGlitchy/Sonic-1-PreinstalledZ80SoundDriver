...
zInputLoop:
	...
	cp	bgm__First			; Is it before first music?
	jr	c,zQueueNext			; If so, jump
	cp	flg__First			; Is it a special command?
	jr	nc,zQueueItem			; If so, jump
	sub	sfx__First			; Subtract first SFX index
...
;    endif
	...
	cp	bgm__Last			; is it music (less than index 20)?
	jr	c,zPlayMusic			; if yes, play it
	cp	sfx__Last			; is it a sound effect (less than index 71)?
	jp	c,zPlaySound_CheckRing		; if yes, play it
	cp	flg__First			; is it after the last regular sound but before the first special sound command (between 71 and 78)?
	...
	sub	flg__First			; convert index 78-7D to a lookup into the following jump table
...
zPlayMusic:
	...
	cp	bgm_ExtraLife		; If NOT 1-up sound...
...
zBGMLoad:
	...
	sub	bgm__First			; Make it a zero-based entry ...
...
zPlaySound:
	...
	ld	a,c				; 'c' -> 'a'
	sub	sfx__First			; Bring 'a' down to index value
...
; zbyte_1214
zSpedUpTempoTable:
	db	s1TempotoS2Z80(7h)
        db	s1TempotoS2Z80(72h)
        db	s1TempotoS2Z80(73h)
        db	s1TempotoS2Z80(26h)
	db	s1TempotoS2Z80(15h)
        db	s1TempotoS2Z80(8h)
        db	s1TempotoS2Z80(0FFh)
        db	s1TempotoS2Z80(5h)
	db 	s1TempotoS2Z80(8h)	; Special Stage
	db 	s1TempotoS2Z80(5h)	; Title Screen
	db 	s1TempotoS2Z80(7h)	; Ending
	db 	s1TempotoS2Z80(5h)	; Boss
	db 	s1TempotoS2Z80(8h)	; FZ
	db 	s1TempotoS2Z80(4h)	; Sonic Got Through
	db 	s1TempotoS2Z80(0Bh)	; Game Over
	db 	s1TempotoS2Z80(8h)	; Continue Screen
	db 	s1TempotoS2Z80(0Dh)	; Credits
	db 	s1TempotoS2Z80(2h)	; Drowning
	db 	s1TempotoS2Z80(6h)	; Get Emerald
...
dac_sample_metadata macro label,sampleRate
	...
	dac_sample_metadata zDACPtr_Kick,8250	; 86h
	dac_sample_metadata zDACPtr_Kick,8250	; 87h
